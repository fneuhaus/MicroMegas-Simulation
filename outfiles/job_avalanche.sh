...
find ${WD} -regextype posix-egrep -regex '^.*particleconversion_[0-9]+_drift\.root$' | parallel --colsep '_drift.root' -j 64 --delay 1 --progress --no-notice "$AVAL_EXEC {1}_drift.root {1}_aval.root > {1}_aval.log"
